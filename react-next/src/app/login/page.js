...
export default function LoginPage() {
  ...
  async function handleRegister(e) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(res.status === 500 ? 'Erro interno. Tente novamente.' : (data.detail ?? 'Erro ao cadastrar.'))
  ...
  return (
    <div className="min-h-screen md:h-screen flex md:overflow-hidden">
      ...
      <div className="hidden md:flex flex-1 relative bg-sidebar flex-col justify-between px-16 py-14 overflow-hidden">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setShowPass(p => !p)}
                    className="cursor-pointer absolute right-3.5 top-1/2 -translate-y-1/2 text-white/50 hover:text-white/80 transition-colors">
                    {showPass
...
const btnCls = `cursor-pointer w-full py-3 rounded-lg bg-white text-primary font-bold text-sm
