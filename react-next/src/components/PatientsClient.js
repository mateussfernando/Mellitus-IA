...
function statusOf(patient) {
  ...
  if (!lastExam) return { key: 'none', label: 'Sem exames', dot: 'bg-slate-300', text: 'text-text-muted', badge: 'bg-slate-100 text-slate-500', accent: 'bg-slate-200' }
  const alerts = generateAlerts([lastExam], patient.sexo)
  if (alerts.some(a => a.level === 'critical')) return { key: 'crit', label: 'Atenção', dot: 'bg-red-500', text: 'text-red-700', badge: 'bg-red-100 text-red-700', accent: 'bg-red-500' }
  if (alerts.some(a => a.level === 'warning')) return { key: 'warn', label: 'Revisar', dot: 'bg-amber-500', text: 'text-amber-700', badge: 'bg-amber-100 text-amber-700', accent: 'bg-amber-400' }
  return { key: 'ok', label: 'Normal', dot: 'bg-emerald-500', text: 'text-emerald-700', badge: 'bg-emerald-100 text-emerald-700', accent: 'bg-emerald-500' }
}
...
export default function PatientsClient({ patients, stats }) {
  ...
  const [examPatient,    setExamPatient]    = useState(null)
  ...
  return (
    <div className="p-4 md:p-8 pt-16 md:pt-8">
      ...
      {/* Controles: busca + toggle de visualização */}
      <div className="flex items-center gap-3 mb-5">
        ...
        <div className="hidden sm:flex items-center bg-surface border border-border rounded-xl p-1">
          {[
            { k: 'cards', d: 'M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6Zm0 9.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25Zm9.75-9.75A2.25 2.25 0 0 1 15.75 3.75H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6Zm0 9.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18v-2.25Z' },
            { k: 'table', d: 'M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5' },
          ].map(b => (
            <button
              key={b.k}
              onClick={() => setView(b.k)}
              className={`cursor-pointer p-2 rounded-lg transition-colors ${view === b.k ? 'bg-primary text-white' : 'text-text-muted hover:text-text'}`}
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d={b.d} /></svg>
            </button>
          ))}
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} className="group relative bg-surface rounded-2xl border border-border overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all">
                <div className={`absolute top-0 inset-x-0 h-1 ${st.accent}`} />
                <div className="p-5">
                  {/* topo */}
                  <div className="flex items-start justify-between mb-4">
                    <button onClick={() => setHistoryPatient(p)} className="cursor-pointer flex items-center gap-3 text-left">
                      <div className="w-11 h-11 rounded-full bg-gradient-to-br from-primary to-primary-dark flex items-center justify-center shrink-0 text-white font-bold">
                        {p.name[0]?.toUpperCase()}
                      </div>
                      <div>
                        <p className="font-semibold text-text leading-tight group-hover:text-primary transition-colors">{p.name}</p>
                        <p className="text-xs text-text-muted mt-0.5">
                          {p.sexo === 'MASCULINO' ? '♂ Masculino' : '♀ Feminino'} · {calcAge(p.birth_date)} anos
                        </p>
                      </div>
                    </button>
                    <span className={`flex items-center gap-1.5 text-[11px] font-semibold px-2.5 py-1 rounded-full ${st.badge}`}>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    <div className="bg-bg rounded-xl px-3 py-2.5">
                      <p className="text-[10px] text-text-muted uppercase tracking-wide">Exames</p>
                      <p className="text-lg font-bold text-text leading-tight">{p._count?.examResults ?? 0}</p>
                    </div>
                    <div className="bg-bg rounded-xl px-3 py-2.5">
                      <p className="text-[10px] text-text-muted uppercase tracking-wide">Glicemia</p>
                      <p className="text-lg font-bold text-text leading-tight">
                        {lastExam?.values?.glicemia != null ? Number(lastExam.values.glicemia).toFixed(0) : '—'}
                        <span className="text-[10px] font-normal text-text-muted ml-1">mg/dL</span>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setExamPatient(p)}
                      className="cursor-pointer flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-primary hover:bg-primary-dark text-white text-sm font-semibold transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="cursor-pointer px-3 py-2 rounded-lg text-sm font-semibold text-primary bg-primary-light hover:bg-primary hover:text-white transition-colors"
                    >
                      Histórico
                    </button>
                    <button
                      ...
                      title="Editar"
                      className="cursor-pointer p-2 rounded-lg text-text-muted hover:text-text hover:bg-bg transition-colors"
                    >
                    ...
                    <tr key={p.id} className="hover:bg-bg/60 transition-colors">
                      <td className="px-5 py-4">
                        <button onClick={() => setHistoryPatient(p)} className="cursor-pointer text-left hover:text-primary transition-colors">
                          ...
                          <p className="text-xs text-text-muted mt-0.5">{calcAge(p.birth_date)} anos</p>
                        </button>
                      </td>
                      <td className="px-5 py-4">
