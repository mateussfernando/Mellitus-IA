...
export default function NewPatientModal({ isOpen, onClose, onSuccess }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!form.sexo) { setError('Selecione o sexo do paciente.'); return }
    setLoading(true)
    setError('')
    ...
    const body = {
      ...
      // Gestações só se aplica a mulheres; homens vão como 0 (default do modelo)
      ...(form.sexo === 'mulher' && form.gestacoes && { gestacoes: Number(form.gestacoes) }),
    ...
    try {
      ...
      if (!res.ok) throw new Error(res.status === 500 ? 'Erro interno. Tente novamente.' : (data.detail ?? 'Erro ao salvar paciente.'))
      setForm(EMPTY)
      onSuccess(data)
    } catch (err) {
