...
export default function PatientDetailClient({ patient, risco }) {
  ...
  async function deleteExam(exam) {
    if (!window.confirm('Remover este exame? Esta ação não pode ser desfeita.')) return
                  ...
                  <div key={exam.id} className="rounded-2xl border border-slate-200 overflow-hidden">
                    <div className="flex items-center justify-between px-5 py-3 bg-slate-50 border-b border-slate-200">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="text-xs text-slate-400 mr-1">{new Date(exam.exam_date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                        <button
                          onClick={() => setEditingExam(exam)}
                          title="Editar exame"
                          className="cursor-pointer p-1.5 rounded-lg text-slate-400 hover:text-primary hover:bg-white transition-colors"
                        >
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125" /></svg>
                        </button>
                        <button
                          ...
                          title="Remover exame"
                          className="cursor-pointer p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-white transition-colors disabled:opacity-50"
                        >
